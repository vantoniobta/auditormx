...
(function($){
	...
	$.fn.upload  = function(options) {
		...
		// changes
		if(this.opts.change){
			document.getElementById(this.opts.input).addEventListener("change", that.uploadFile.bind(this), false);
		}else{
			// submit
	...
	$.fn.uploadProgress = function(evt) {
	  if (evt.lengthComputable) {
	     ...
	     if(this.opts.showpercent){
	     	...
	     	$('#'+this.opts.progressNum).html( 'Progress: (' + percentComplete.toString() + '%)');
	     }
	     if(this.opts.progressivebar){
	 	 ...
	 	 if( percentComplete == 100 ){
	     	if( that.opts.wait ){
	     		setTimeout(function(){
	     			$('#'+that.opts.progressNum).html('Uploading file...');
	  			}, 1000);
	     	}
	     }
	  }else {
